  ...
  <div class="home-category" @mouseleave="categoryId = null">
    <ul class="menu">
      <li :class="{ active: categoryId && categoryId === item.id }" v-for="item in menuList" :key="item.id" @mouseenter="categoryId = item.id">
        ...
        <!-- 骨架 -->
        <template v-else>
          <tm-skeleton width="60px" height="18px" bg="rgba(255, 255, 255, 0.2)" style="margin-right: 5px" />
          <tm-skeleton width="50px" height="18px" bg="rgba(255, 255, 255, 0.2)" />
        </template>
      </li>
    </ul>
    <!-- 弹层 -->
...
export default {
  ...
  setup () {
    ...
    // 获取品牌数据。在 setup 上尽量不用使用 async，因为它默认不支持 async。
    findBrand().then(data => {
