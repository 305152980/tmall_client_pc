<template>
  <div class="order-item">
    <div class="head">
      ...
      <a @click="$emit('onCelete', order)" href="javascript:;" class="del" v-if="[5, 6].includes(order.orderState)">删除</a>
    </div>
    <div class="body">
      ...
      <div class="column state">
        ...
        <!-- 待收货  查看物流 -->
        <p @click="$emit('onLogistics', order)" v-if="order.orderState === 3"><a class="green" href="javascript:;">查看物流</a></p>
        <!-- 待评价  评价商品 -->
      ...
      <div class="column action">
        ...
        <!-- 已完成：查看详情，再次购买，申请售后。 -->
        <!-- 已取消：查看详情。 -->
        <tm-button @click="$router.push(`/member/pay?orderId=${order.id}`)" v-if="order.orderState === 1" type="primary" size="small">立即付款</tm-button>
        <tm-button @click="$emit('onConfirm', order)" v-if="order.orderState === 3" type="primary" size="small">确认收货</tm-button>
        <p><a @click="$router.push(`/member/order/${order.id}`)" href="javascript:;">查看详情</a></p>
        <p @click="$emit('onCancel', order)" v-if="order.orderState === 1"><a href="javascript:;">取消订单</a></p>
...
export default {
  ...
  emits: ['onCancel', 'onCelete', 'onConfirm', 'onLogistics'],
  setup (props) {
